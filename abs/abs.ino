...
long abs_func(float wheel_speed, int user_position, float vehicle_speed) { // returns optimal braking position based on controls
  ...
  if (user_position <= 71) { // no brake input
    current_abs_position = user_position;  //current abs position is just 71/70
    abs_activate = false;
    return user_position;
  }

  else if (abs_activate == true) {
    current_abs_position = min(abs_position, user_position);  // sets current abs to either the calculated abs position or user's position
    return min(current_abs_position, user_position);  // 
